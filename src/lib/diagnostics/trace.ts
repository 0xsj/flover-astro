...
export function createTrace(
	...
	function start(stage: DiagnosticStage) {
		...
		return (result: { ok: true } | { ok: false; error: Failure }, thrown = false) => {
			try {
				const ended = now();
				const finish = {
					...base,
				...
				if (result.ok) emit({ ...finish, outcome: 'success' });
					...
					emit({
						...finish,
						outcome:
							thrown ? 'thrown' : result.error.kind === 'canceled' ? 'canceled' : 'failure',
	...
	return {
		...
		runSync<T, E extends Failure>(stage: DiagnosticStage, work: () => Result<T, E>): Result<T, E> {
			const finish = start(stage);
			try {
				const result = work();
				finish(result);
