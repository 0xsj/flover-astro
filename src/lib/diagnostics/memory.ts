...
export type DiagnosticEntry = Readonly<{ sequence: number; event: DiagnosticEvent }>;
export type DiagnosticSnapshot = Readonly<{
	entries: readonly DiagnosticEntry[];
	dropped: number;
}>;

const EMPTY: DiagnosticSnapshot = Object.freeze({ entries: Object.freeze([]), dropped: 0 });

const label = (value: string) => value.replace(/[\u0000-\u001f\u007f]/g, '').slice(0, 96);

/** Project a typed event again so retained diagnostics cannot become a bag of
 * sensitive data through accidental extra runtime properties. */
function project(event: DiagnosticEvent): DiagnosticEvent {
	...
	if (event.event === 'started') return Object.freeze({ ...base, event: 'started' });
	const finish = { ...base, event: 'finished' as const, durationMs: event.durationMs };
	if (event.outcome === 'success') return Object.freeze({ ...finish, outcome: 'success' });
...
export function createMemoryDiagnostics(options: { capacity?: number } = {}) {
	...
	const capacity = Number.isInteger(requested) && requested > 0 ? Math.min(requested, 5000) : 200;
	let snapshot = EMPTY;
	let sequence = 0;
	...
	const port: DiagnosticsPort = {
		record(event) {
			const entry = Object.freeze({ sequence: ++sequence, event: project(event) });
