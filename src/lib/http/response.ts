...
/** Undefined rejects the response. Readers also project wire data into DTOs. */
...
export function responseDecoder<T>(contract: string, read: ResponseReader<T>): ResponseDecoder<T> {
	const decode = (value: unknown): Result<T, Fails<'internal'>> => {
		try {
			const decoded = read(value);
			if (decoded !== undefined) return ok(decoded);
		} catch {
			/* Reader exceptions are not public response details. */
		}
		return err(
	...
	return (value, trace) => (trace ? trace.runSync('decode', () => decode(value)) : decode(value));
}
