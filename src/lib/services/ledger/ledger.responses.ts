...
function readEntry(value: unknown): AuditEntry | undefined {
	...
	if (
		...
		(entry.scope !== 'system' && entry.scope !== 'account' && entry.scope !== 'session')
	) {
		return undefined;
	}
...
const readFacets = responseArray((value): { facet: string; total: number } | undefined => {
...
export const decodeAuditPage = responseDecoder('activity page', (value): AuditPage | undefined => {
	const page = responseObject(value);
	if (!page) return undefined;
	const entries = readEntries(page.entries);
	if (!entries || (page.next !== undefined && !responseText(page.next))) return undefined;
	const facets = page.facets === undefined ? undefined : readFacets(page.facets);
