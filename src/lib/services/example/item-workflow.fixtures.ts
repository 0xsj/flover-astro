import { conflict, err, invalid, notFound, ok, type Result } from '../../kernel';
import {
	responseArray,
	responseDecoder,
	responseObject,
...
export const decodeItemDatabase = responseDecoder(
	...
	(raw): ItemDatabase | undefined => {
		...
		const items = responseArray(readEditableItem)(value?.items);
		const receipts = responseArray(readItemReceipt)(value?.receipts);
		if (
			!value ||
...
export function initialItems(generation: string): ItemDatabase {
	...
	return {
		...
		items: names.map((name, index) => ({
			id: ids[index],
			name,
			host: `${ids[index]}.${index % 3 === 0 ? 'internal' : 'example.com'}`,
			revision: 1
		}))
	};
}
		...
		},
		{
			...
			handle(_request, match) {
				return persistence.read().andThen((data) => {
					const item = data.items.find((value) => value.id === decodeURIComponent(match[1]));
				...
				return persistence.read().andThen((data) => {
					...
					if (previous)
						return previous.id === attempt.id &&
							...
							? ok(previous)
							: err(conflict('This operation id belongs to different changes.'));
					...
					const stored = persistence.write({
						...data,
						items: data.items.map((value) => (value.id === item.id ? item : value)),
			...
			handle(_request, match) {
				return persistence.read().andThen((data) => {
					const receipt = data.receipts.find(
						(value) => value.operationId === decodeURIComponent(match[1])
					);
					return receipt
						? ok(receipt)
						: err(
								notFound('This operation did not commit and cannot commit later.', {
									type: NO_ITEM_SAVE
								})
						  );
				});
