...
export type ItemSaveFailure = TransportFailure | Fails<'invalid' | 'conflict' | 'not_found'>;
export const NO_ITEM_SAVE = 'no_item_save';
export const sameItemDraft = (a: NewItem, b: NewItem) => a.name === b.name && a.host === b.host;
...
export function readItemReceipt(raw: unknown): ItemReceipt | undefined {
	const value = responseObject(raw);
	const attempt = readItemAttempt(raw);
...
export function validateItemDraft(draft: NewItem): Result<NewItem, Fails<'invalid'>> {
	...
	return Object.keys(fields).length ? err(invalid('Check the item details.', fields)) : ok(draft);
}
...
const asSave = narrow('invalid', 'conflict', 'not_found');

export async function readItem(client: HttpClient, id: string, options?: CallOptions) {
	const result = (
	...
	return result.ok && result.value.id !== id
		? err(internal('The returned item did not match this address.', { type: 'invalid_response' }))
		: result;
}
...
export async function updateItem(
	...
	if (
		...
		return err(internal('The receipt did not match the submitted item.', { type: 'invalid_response' }));
	}
	return result;
}
...
export async function findItemSave(
	...
	const result = await optional(
		(
			await client.get<unknown>(`/item-saves/${encodeURIComponent(operationId)}`, options)
		)
			.andThen((value) => decodeReceipt(value, options?.trace))
			.mapErr(asRead),
	...
	return result.ok && result.value && result.value.operationId !== operationId
		? err(internal('The receipt did not match the requested operation.', { type: 'invalid_response' }))
		: result;
}
