...
const decodeReceipt = responseDecoder('note receipt', (value): NoteReceipt | undefined => {
...
export async function saveNote(
	...
	if (!attempt.draft.title.trim()) return err(invalid('A title is required.', { title: 'Enter a title.' }));
	const result = (
	...
	if (
		...
		return err(internal('The save receipt did not match the submitted note.', { type: 'invalid_response' }));
	return result;
}
...
export async function findNoteSave(
	...
	const result = await optional(
		(
			await client.get<unknown>(`/example/note-saves/${encodeURIComponent(operationId)}`, {
				signal: options?.signal,
				trace: options?.trace
			})
		)
	...
	if (result.ok && result.value !== null && result.value.operationId !== operationId)
		return err(internal('The save receipt did not match the requested operation.', { type: 'invalid_response' }));
	return result;
}
