	...
	const routes: MemoryRoute[] = [
		{
			...
			handle(request) {
				...
				if (
					...
					return err(invalid('The note is incomplete.', { title: 'Enter a title.' }));
				const previous = receipts.get(attempt.operationId);
				if (previous)
					return previous.draft.title === draft.title && previous.draft.body === draft.body
		...
		},
		{
			...
			handle(_request, match) {
				...
				return receipt
					...
					: err(
							notFound('No save was recorded for this operation.', { type: NO_NOTE_SAVE })
					  );
			}
		}
	];
