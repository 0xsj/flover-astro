...
export const decodeCapabilities = responseDecoder(
	...
	(raw): Capabilities | undefined => {
		...
		const grants = responseObject(value?.grants);
		if (
			!value ||
		...
		for (const [name, rawGrant] of Object.entries(grants)) {
			const decision = responseObject(rawGrant);
...
export function capability(snapshot: Capabilities, name: string): Capability {
	return Object.hasOwn(snapshot.grants, name)
		...
		: { allowed: false, reason: 'This action is not available for this account.' };
}
...
export async function readCapabilities(
	...
	const result = (
		await client.get<unknown>(`/capabilities/${encodeURIComponent(resource)}`, options)
	).andThen((value) => decodeCapabilities(value, options?.trace));
	return result.ok && (result.value.subject !== subject || result.value.resource !== resource)
		? err(internal('Capabilities belong to another scope.', { type: 'invalid_response' }))
		: result;
}
