...
export type ResponseMode = 'valid' | 'malformed' | 'partial' | 'unavailable' | 'empty' | 'held';
export type SaveMode = 'success' | 'refused' | 'lost-response' | 'not-delivered';
 ...
 * Imported by recipes only, so other consumers of lib/root do not ship them. */
export async function readResponseExample(
	...
	const sequence = createSequenceClient(
		createMemoryClient({
			latencyMs: 0,
			routes: [{ method: 'GET', pattern: /^\/items$/, handle: () => ok(items) }],
			getCorrelationId: () => trace?.correlationId
		}),
		effect ? [{ request: 'GET /items', effect }] : []
	);
	try {
		return await listItems(withDiagnostics(sequence.client), 'response-demo', { signal, trace });
	} finally {
		sequence.dispose();
	}
}
					...
					handle: (request) =>
						ok(
							request.params?.workspace === 'earlier'
								? [{ id: 'earlier', name: 'Earlier selection', host: 'earlier.example.com' }]
								: [{ id: 'newer', name: 'Newer selection', host: 'newer.example.com' }]
						)
				}
			]
		}),
	...
	return {
		...
		async save(attempt: NoteAttempt, signal: AbortSignal) {
			const effects: Record<SaveMode, SequenceStep['effect'] | undefined> = {
				...
				'not-delivered': {
					kind: 'fail',
					failure: unavailable('The request could not reach the simulated server.')
				}
			};
		...
		find: (id: string, signal: AbortSignal) =>
			failChecks
				? Promise.resolve(
						err(unavailable('The save receipt could not be checked. Try checking again.'))
				  )
