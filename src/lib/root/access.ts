...
export function createAccessExample(subject: string, latencyMs = 200) {
	...
	const decision = (allowed: boolean): Capability =>
		allowed
			...
			: {
					allowed: false,
					reason: 'Your access to this workspace was removed. Ask its owner to restore it.'
			  };
			...
			},
			{
				...
				handle: () =>
					view && edit
						...
						: err(forbidden('The server refused this action because your access changed.'))
			}
		]
	});
	return {
		...
		update: (signal: AbortSignal) => updateProtectedWorkspace(client, { signal }),
