...
import { err, internal, invalid, ok, timeout, unavailable, type Result } from '../kernel';
import { createBrowserStorage, createDocument, type StoragePort } from '../storage';
...
const decodeCheckpoints = responseDecoder(
	...
	(raw): Checkpoints | undefined => {
		...
		const entries = responseArray((entryRaw): ItemCheckpoint | undefined => {
			const value = responseObject(entryRaw);
			const item = readEditableItem(value?.item);
			const draft = readItemDraft(value?.draft);
			const baseline = readItemDraft(value?.baseline);
			const confirmed = value?.confirmed === null ? null : readItemReceipt(value?.confirmed);
			const attempt = value?.attempt === null ? null : readItemAttempt(value?.attempt);
			if (
				!item ||
				!draft ||
				!baseline ||
				confirmed === undefined ||
				attempt === undefined ||
				(confirmed && (confirmed.id !== item.id || !sameItemDraft(baseline, confirmed.draft))) ||
				(attempt &&
					(attempt.id !== item.id ||
						attempt.expectedRevision !== (confirmed?.item.revision ?? item.revision)))
			)
				return undefined;
...
export function createItemWorkflow(
	...
	let saveMode: ItemSaveMode = 'success';
	let checksFail = false;

	function readData(): Result<ItemDatabase> {
		const value = database.read();
		if (!value.ok) return err(value.error);
		return value.value.state === 'found' && value.value.value.generation === generation
			? ok(value.value.value)
			: err(missing());
	}

	const fixtures = createItemFixtures({ read: readData, write: database.write });
	async function call<T>(
		operation: string,
		request: string,
		effect: SequenceStep['effect'] | undefined,
		stage: 'operation' | 'recovery',
		work: (client: HttpClient, trace: ReturnType<typeof createTrace>) => Promise<Result<T>>
	): Promise<Result<T>> {
		...
		active.add(sequence);
		try {
			return await trace.run(stage, () => work(withDiagnostics(sequence.client), trace));
		} finally {
	...
	return {
		...
		initialize(): Result<void> {
			...
			if (saved.value.state === 'found' && saved.value.value.generation !== generation)
				return err(missing());
			return ok(undefined);
		},
		reset(): Result<void> {
		...
		loadDraft(id: string): Result<ItemCheckpoint | null> {
			...
			return ok(saved.value.value.entries.find((entry) => entry.item.id === id) ?? null);
		},
		checkpoint(entry: ItemCheckpoint): Result<void> {
			...
			const saved = drafts.read();
			if (!saved.ok) return err(saved.error);
			const entries = saved.value.state === 'missing' ? [] : saved.value.value.entries;
			if (saved.value.state === 'found' && saved.value.value.generation !== generation)
				return err(missing());
			return drafts.write({
				generation,
				entries: [...entries.filter((value) => value.item.id !== entry.item.id), entry]
			});
		},
		...
		list(signal: AbortSignal) {
			return call('items.list', 'GET /items', readEffect(), 'operation', (client, trace) =>
				listItems(client, 'workflow', { signal, trace })
			);
		},
		...
		save(attempt: ItemAttempt, signal: AbortSignal) {
			const effects: Record<ItemSaveMode, SequenceStep['effect'] | undefined> = {
				...
				'lost-response': {
					kind: 'lose-response',
					failure: timeout('The save response did not arrive. Check whether the item was saved.')
				},
				'not-delivered': {
					kind: 'fail',
					failure: unavailable('The request did not reach the simulated server.')
				},
				held: { kind: 'hold-response', gate: 'save-response' }
			};
			const mode = saveMode;
			if (mode === 'held') heldSaves++;
			return call(
				'items.save',
