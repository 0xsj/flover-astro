...
export function createJobExample(kind: Job['kind'] = 'export', latencyMs = 200) {
	let job: Job = { id: `${kind}-demo`, revision: 1, kind, state: 'queued' };
	let connected = true;
	let requested = false;
	let cancelMode: CancelMode = 'accept';
	...
	const client = createMemoryClient({
		...
		routes: [
			...
			},
			{
				...
				pattern: /^\/jobs\/[^/]+\/cancel$/,
				handle: () => {
					if (!connected) return err(unavailable('The cancellation could not be confirmed.'));
					...
					return cancelMode === 'lost-response'
						? err(timeout('The cancellation response was lost. Refresh the job to learn its outcome.'))
	...
	return {
		...
		cancel: (id: string, signal: AbortSignal) => cancelJob(client, id, { signal }),
			...
			job = requested
				...
				: job.state === 'queued'
					...
					: job.state === 'running' && job.progress === null
						...
						: { ...base, state: 'completed', summary: '240 records processed successfully.' };
			emit();
		},
		fail() {
			if (!terminalJob(job)) {
