...
/** An isolated session-expiry specimen for the cookbook. It composes the item
 * workflow with an identity endpoint without reading the real session cookie. */
export function createSessionExample(
		...
		routes: [
			{
				...
				handle: () =>
					verifyUnavailable
						? err(unavailable('Sign-in verification is temporarily unavailable.'))
						: identity === null
							? err(unauthenticated('Sign in to continue.'))
							: ok({ id: identity, name: 'Demo account', email: 'demo@example.com' })
			}
		]
	});
	...
	return {
		...
		find(operationId: string, signal: AbortSignal) {
			const failure = refused();
			return failure ? Promise.resolve(err(failure)) : items.find(operationId, signal);
		},
		verify: async (signal: AbortSignal) =>
			(await currentUser(client, { signal })).map((user) => ({ accountId: user.id })),
		expire() {
			identity = null;
		},
