...
export type QuerySchema<T> = {
	...
	write: (value: T, base?: URLSearchParams | string) => Result<URLSearchParams, Fails<'invalid'>>;
	update: (
...
export function querySchema<const S extends Shape>(shape: S): QuerySchema<QueryState<S>> {
	type State = QueryState<S>;
	const entries = Object.entries(shape);
	for (const [key, codec] of entries) {
		const encoded = codec.format(codec.defaultValue);
		if (!key || encoded === undefined || codec.parse(encoded) !== codec.defaultValue)
			throw new Error('Query schema defaults must round-trip through their codecs.');
	}
	const defaults = Object.freeze(
		Object.fromEntries(entries.map(([key, codec]) => [key, codec.defaultValue]))
	) as State;
	...
	const read = (query: URLSearchParams | string): QueryRead<State> => {
		...
		for (const [key, codec] of entries) {
			...
			if (decoded === undefined) issues.push({ key, message: issue(key, codec) });
	...
	const write = (
		...
		for (const [key, codec] of entries) {
			...
			try {
				...
				const canonical = encoded === undefined ? undefined : codec.parse(encoded);
				if (encoded === undefined || canonical === undefined) {
					fields[key] = `Choose ${codec.expected}.`;
					continue;
				}
				if (encoded !== codec.format(codec.defaultValue)) params.append(key, encoded);
			} catch {
				fields[key] = `Choose ${codec.expected}.`;
			}
		}
