...
export type LocaleContext = { locale: string; timeZone: string; currency: string };
...
const bad = (message: string) => err(invalid(message, {}, { type: 'format_invalid' }));

/** Construct all presentation formatters from an explicit context. */
export function createFormatters(context: LocaleContext): Result<Formatters> {
	try {
		if (
			...
			return bad('Choose a supported locale, time zone and three-letter currency.');
		}
		...
		return ok({
			context: { ...context },
			number: (value) =>
				Number.isFinite(value) ? ok(number.format(value)) : bad('A finite number is required.'),
			money: (value) =>
