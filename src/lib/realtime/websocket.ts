...
export type SocketOptions<T> = {
	...
	socket?: (url: string) => Pick<WebSocket, 'addEventListener' | 'removeEventListener' | 'close'>;
	retry?: { attempts: number; baseMs: number; maxMs: number };
};

/** WebSocket adapter; policy/auth closes are terminal, network closes retry. */
export function createWebSocketSource<T>({
	url,
	decode,
	socket = (value) => new WebSocket(value),
	retry = { attempts: 8, baseMs: 250, maxMs: 8000 }
	...
	return {
		subscribe(observer) {
			...
			const dropped = () => unavailable('Live updates are disconnected.', { type: 'socket_disconnected' });
			const reconnect = () => {
				if (disposed) return;
				...
				const retryInMs = Math.min(retry.baseMs * 2 ** attempt, retry.maxMs);
				attempt += 1;
				observer.state({ state: 'reconnecting', attempt, retryInMs, failure: dropped() });
				timer = setTimeout(connect, retryInMs);
			};
			const connect = () => {
				if (disposed) return;
				let connection: ReturnType<typeof socket>;
				try {
				...
				const onMessage = (event: MessageEvent) => {
					...
					try {
						...
						observer.error(invalid('A live update could not be decoded.', {}, { type: 'socket_message' }));
					}
				};
				const detach = () => {
				...
				const onClose = (event: CloseEvent) => {
					...
					if (event.code === 1000 || event.code === 1008 || event.code >= 4000) {
						observer.state({ state: 'closed', ...(event.code === 1000 ? {} : { failure: dropped() }) });
					} else reconnect();
				};
				const onError = () => {
