...
import { terminalJob, type CancelReceipt, type Job } from '../services/jobs';
...
export function createJobObserver(id: string, port: JobPort) {
	...
	const update = (patch: Partial<JobObserverState>) => store.set({ ...store.get(), ...patch });
			...
			if (
				...
				(previous &&
					(job.kind !== previous.kind ||
						...
						(job.revision === previous.revision && snapshotKey(job) !== snapshotKey(previous)) ||
						(terminalJob(previous) && snapshotKey(job) !== snapshotKey(previous)) ||
						(previous.state === 'running' && job.state === 'queued')))
			)
				result = err(internal('The job snapshot is obsolete or inconsistent.', { type: 'invalid_response' }));
		}
	...
	return {
		...
		async cancel() {
			...
			if (!state.job || terminalJob(state.job) || state.cancellation !== 'idle') return;
			...
			if (result.ok && result.value.id !== id)
				result = err(internal('Cancellation acknowledged another job.', { type: 'invalid_response' }));
			if (result.ok) update({ cancellation: result.value.accepted ? 'accepted' : 'too-late' });
		...
		dispose() {
			...
			cancelGeneration++;
			canceler?.abort();
			if (store.get().cancellation === 'pending') update({ cancellation: 'unknown' });
		}
	};
}
