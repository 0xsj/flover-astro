...
/** For an uncached, replaceable read. Cached resources use a query layer.
 * The last invocation owns the result even if a transport ignores abort.
 * Previous data stays inside this instance's identity/scope. */
export function createLatestRead<Input, Value>(
	...
	return {
		...
		async run(input: Input) {
			...
			} else settled = { state: 'failed', failure: result.error, previous };
			store.set(settled);
		},
		cancel() {
			generation++;
