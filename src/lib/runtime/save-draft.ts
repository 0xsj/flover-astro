import { asFailure, canceled, err, internal, ok, type Failure, type Result } from '../kernel';
...
export function createSaveDraft<D, A extends DraftAttempt<D>, R extends DraftAttempt<D>>(
	port: DraftPort<A, R>,
	initial: D,
	options: {
	...
	function transition(state: DraftState<D, A, R>): Result<void> {
		...
		store.set({ ...state, checkpointFailure: persisted.ok ? undefined : persisted.error });
		return persisted;
	}
	const updatePhase = (phase: DraftPhase<A>) => transition({ ...store.get(), phase });
const uncertain = (attempt: A, failure: Failure) => updatePhase({ state: 'unknown', attempt, failure });
	...
	return {
		...
		edit(draft: D) {
			transition({ ...store.get(), draft: structuredClone(draft) });
		},
		checkpoint() {
			return transition(store.get());
		},
		discard() {
			if (['saving', 'checking', 'unknown'].includes(store.get().phase.state)) return false;
			transition({
				...store.get(),
		...
		async save() {
			...
			if (!checkpoint.ok) {
				updatePhase({ state: 'refused', failure: checkpoint.error, submitted: attempt });
				return;
			}
			let result: Result<R>;
			try {
			...
			else if ((options.refused ?? ((failure) => failure.kind === 'invalid'))(result.error))
				updatePhase({ state: 'refused', failure: result.error, submitted: attempt });
		...
		cancel() {
			...
			if (phase.state === 'saving' || phase.state === 'checking')
				uncertain(
					phase.attempt,
					canceled('The operation was interrupted. Check its outcome before saving again.')
				);
		}
	};
}
