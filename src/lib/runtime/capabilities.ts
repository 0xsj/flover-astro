...
import { capability, type Capabilities, type Capability } from '../services/access';
	...
	return {
		...
		decide(name: string): Capability {
			...
			return state.state === 'ready'
				...
				: {
						allowed: false,
						reason: 'Access has not been verified. Refresh permissions to continue.'
				  };
		},
		async refresh() {
			controller?.abort();
			...
			if (
				...
				result = err(
					internal('The permission snapshot is obsolete or belongs to another scope.', {
