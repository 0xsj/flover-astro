...
import type { DocumentSchema, StoragePort, Stored, StoredDocument } from './port';

type Envelope = { version: number; value: unknown };
const malformed = () => invalid('The saved document is invalid.', {}, { type: 'storage_invalid' });
...
export function createDocument<T>(
	...
	function read(): Result<Stored<T>> {
		...
		if (migrated) {
			if (!schema.migrate)
				return err(
					conflict('This saved version needs a migration or an explicit reset.', {
						type: 'storage_version'
					})
				);
			try {
				value = schema.migrate(saved.value.value, saved.value.version).andThen(decode);
			} catch {
				return err(malformed());
			}
		} else value = decode(saved.value.value);
		return value.map((decoded) => ({ state: 'found' as const, value: decoded, migrated }));
	}
	return {
		read,
		write(value) {
			if (!Number.isSafeInteger(schema.version) || schema.version < 1) return err(malformed());
			...
			if (previous.value !== null) {
				...
				if (existing.ok && existing.value.version > schema.version) return err(future());
			}
