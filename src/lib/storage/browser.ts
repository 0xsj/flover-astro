...
function failure(error: unknown): Failure {
	...
	if (name === 'SecurityError')
		return forbidden('Browser storage is blocked.', { type: 'storage_blocked' });
...
export function createBrowserStorage(area: 'local' | 'session' = 'local'): StoragePort {
	...
	return {
		...
		write: (key, value) => change(key, (storage) => storage.setItem(key, value)),
		...
		subscribe(key, listener) {
			return access((storage) => {
				...
				const group = listeners.get(storage) ?? new Set<(key: string) => void>();
