...
function blockMarkdown(block: ManualBlock, origin: string): string {
	switch (block.kind) {
		...
		case 'links':
			return block.items
				.map((item) => `- [${item.label}](${new URL(item.href, origin).href}) — ${item.description}`)
				.join('\n');
		case 'sources':
...
export function manualMarkdown(origin: string): string {
	return (
		[
			'# Flover user manual',
			"A practical guide to the template's layers, Result/Failure model, adapters and recovery behavior. App links point to the Astro instance that produced this download. Repository references are paths inside a Flover checkout.",
			chapters.map((chapter, index) => `${index + 1}. [${chapter.title}](#${chapter.slug})`).join('\n'),
